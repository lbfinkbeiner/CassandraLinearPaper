\chapter{Description of Emulator Code and Pipeline}
\label{chap: cassL}
...
machinery which leads to an emulator. CL represents a full emulation pipeline 
from the generation of training and 
...
emulator. For an explanation of the abbreviations used here, see 
table~\ref{tab: script_summary}. \selfcomment{The following two paragraphs are 
not very easy to read, but I'm not sure how to better structure them.}
...
\selfcomment{Talk about how you got this equation?}
...
described in tables~\ref{tab: CLASSIC_priors} and~\ref{tab: COMET_priors}. The 
two pairs of files share the same priors in $\omega_\nu$, [0, 0.01], and
$\tilde{\sigma}_{12}$, [0.2, 1], and so these are omitted. \selfcomment{Talk a 
little more about the prior ranges, what are their values in words?}
...
\end{tabular}
	\cprotect\caption[``CLASSIC'' priors]{``CLASSIC'' priors. We expect this
 	...
 	statistical analyses. \selfcomment{citation.}}
...
\selfcomment{I'm not referring to table 1.2 here
...
interval [0, 10]. \selfcomment{log spacing would have been better for
redshift}. This interval captures all but the reddest galaxies
in our galaxy redshift surveys. \selfcomment{citation.} Next, these
...
\selfcomment{cite some numbers for this claim!}.
...
\selfcomment{We should make error plots (comparing the two approaches 
...
where \texttt{''} in the third line is a regex matching all parameter names. 
This
...
normalized spectrum; $\bar{y}_i$ is the average of all $\log y_i$; and
$\sigma_{y_i}$ is the standard deviation of all $\log y_i$.
...
\selfcomment{Can we add some material in
section~\ref{sec: gpr_intro}: why is GPR vulnerable in this way?}

\selfcomment{Briefly explain what GPy is doing--don’t treat it like some 
black box.}
...
default emulators CL will build. \selfcomment{Where are the training errors?}
...
\end{tabular}
 \cprotect\caption[Summary of CL Scripts]{The scripts that make up CL.
 	...
 	addressed in detail in these two chapters.}
